...
Приложение на Qt6, отображающее гексагональные ячейки H3 на карте через MapLibre Native Qt и строящее кратчайший путь
через соты h3: с разрешения соты h3 **2–15** до другой соты h3 **2-15** с помощью A*. Для оптимизации поиска пути определяется
какой индекс h3 находится под сотой, которую Вы выбрали и далее ищет путь на разрешении 2, т.е. Например, начальное
разрешение **14**, конечная точка на уровне **4**, то будет путь таков: 
НАЧАЛО (14)→13→12→11→10→9→8→7→6→5→4→3→2 ...→(путь на разрешении 2)→... 2→3→(4) КОНЕЦ.
